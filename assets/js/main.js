(function ($) {
    ...
    */
    ...
    function handleAccordionBox() {
        $('[data-hubitat-accordion]').each(function () {
            var accordion = $(this);
            ...
            //toggle($(this).attr('data-hubitat-accordion') || 0);

            this.accordionToggle = toggle;
        });
    };
    $(window).on('hubitat:handle_accordion_box', handleAccordionBox);
    ...
    $(window).on('hubitat:handle_accordion_box_size', handleAccordionBoxSize);
    ...
    $(window).on('hubitat:handle_counter_box', handleCounterBox);
    ...
    $(window).on('hubitat:handle_contact_forms', handleSubscribeContactForm);
    ...
    $(window).on('hubitat:handle_countdown_box', handleCountdownBox);

    /* ## Cover box */

    function handleCoverBox() {
        $('[data-hubitat-cover-box]').each(function () {
            var box = $(this),
    ...
    $(window).on('hubitat:handle_cover_box', handleCoverBox);
    ...
    $(window).on('hubitat:handle_cover_box_size', handleCoverBoxSize);
    ...
    $(window).on('hubitat:handle_gallery', handleGallery);
    ...
    function handleProgressBar() {
        $("[data-hubitat-progress-bar]:not([data-processed])").each(function () {
            var percent,
                bar = $(this),
                line = bar.find('.line'),
                progressEnd = parseInt(bar.attr("data-hubitat-progress-bar")),
    ...
    $(window).on('hubitat:handle_progress_bar', handleProgressBar);

    /* ## Progress bar size */

    function handleProgressBarSize() {
        $("[data-hubitat-progress-bar][data-processed]").each(function () {
            var bar = $(this);
            var line = bar.find('.line');
            var progressEnd = parseInt(bar.attr("data-hubitat-progress-bar"));
    ...
    function handleSliders(image) {
        ...
        $('[data-hubitat-slider]').each(function () {
            var carousel = $(this);
            var options = $(this).attr('data-hubitat-slider');
        ...
        $('[data-hubitat-slider-simple]').each(function () {
            var carousel = $(this);
            ...
            carousel.clbSlider({
                ...
                setTimeout(function () {
                    $('.hubitat-masonry').masonry();
                }, 250);
            });
        });
    }
    $(window).on('hubitat:handle_sliders', handleSliders);

    function handleFullscreenSlider() {
        var onepage = $('.fullscreen-slider');
        if (onepage.length) {

            if (onepage.find('.hubitat-dynamic-text-sc ')) {
                handleHubitatDynamicTextSc();
            }
    ...
    $(window).on('hubitat:handle_fullscreen_sliders', handleFullscreenSlider);
    ...
    function handleTabBox() {
        $('[data-hubitat-tab-box]').each(function () {
            var box = $(this);
            ...
            function productReviewSmoothScroll(event) {
                ...
                tab.find('.tabNav_link[data-hubitat-tab="reviews"]').addClass('active');
                tab.find('.tabItems_item[data-hubitat-tab-content="reviews"]').addClass('active');
    ...
    $(window).on('hubitat:handle_tab_box', handleTabBox);
    ...
    $(window).on('hubitat:handle_tab_box_size', handleTabBoxSize);
    ...
    function handleQuickviewPopup(items) {
        ...
        link.on("click", function (event) {
            ...
            $.ajax({
                url: hubitatVariables.url,
                data: {
                    action: 'hubitat_product_modal',
    ...
    function handlePortfolio() {
        // Filter
        $('[data-hubitat-portfolio-grid]').each(function () {
            ...
            $('body').on('hubitat:lazy_load_complete', function () {
    ...
    $(window).on('hubitat:handle_portfolio', handlePortfolio);
    ...
    $(window).on('hubitat:handle_portfolio_popup', handlePortfolioPopup);
    ...
    function loadLightboxGallery() {
        if (!this.getAttribute('data-lightbox-loaded')) {
            ...
            $(this).find('[data-hubitat-lightbox-image]').each(function () {
                var originalImage = this;
                var image = new Image();
                image.src = originalImage.getAttribute('data-hubitat-lightbox-image');
                image.onload = function () {
                    originalImage.style.backgroundImage = "url(" + image.src + ")";
                    loaded++;

                    if (loaded === $(self).find('[data-hubitat-lightbox-image]').length) {
    ...
    function handleScrollContent() {
        $('[data-hubitat-content-scroll]').each(function () {
            var content = $(this),
                parent = $($(this).attr('data-hubitat-content-scroll')),
            ...
            if (content.data('hubitat-content-scroll') == '#checkout-content-scroll') {
    ...
    jQuery(function ($) {
        ...
        $(document).on('click', '.single_add_to_cart_button', function (e) {
            ...
            if ($thisbutton.is('.single_add_to_cart_button')) {
                ...
                //     var quantity = $variation_form.find('input[name=quantity]').val();
                //     var data = {
                //         action: 'hubitat_ajax_add_to_cart_woo',
                ...
                //         var data = {
                //             action: 'hubitat_ajax_add_to_cart_woo_single',
                //             product_id: product_id,
                //             quantity: quantity
                //         };
                //     } else {
                ...
                if ($('form.cart').hasClass('woo_c-cart-form')) {
                    ...
                    data.action = 'hubitat_ajax_add_to_cart_woo';
                ...
                $.post(wc_cart_fragments_params.ajax_url, data, function (response) {
                    ...
                    if (wc_add_to_cart_params && wc_add_to_cart_params.cart_redirect_after_add == 'yes') {
                        ...
                        $thisbutton.after('<a href="' + hubitatVariables.cart_page + '" class="' + $classes + '">' + hubitatVariables.view_cart + '</a>');
                        ...
                        $('footer').before('<div class="woo_c-message-group"><div class="ajax-cart-response message-box success">' + productName + ' ' + hubitatVariables.add_to_cart_message + '<a class="view_cart_button" href="' + hubitatVariables.cart_page + '">' + hubitatVariables.view_cart + '</a><div class="btn-round btn-round-small btn-round-light clb-close dark-mode-reset" tabindex="0"><i class="ion ion-md-close"></i></div></div></div>');
                    }
                });
                return false;
            } else {
    ...
    function lazyLoad(elem) {
        ...
        if (!elem.data('lazy-load-shortcode')) {
            ...
            data = {
                ...
                paged: currentPage,
                action: 'hubitat_lazy_load_shortcodes'
            };
        ...
        $.ajax({
            ...
            success: function (content) {
                ...
                var metroImages = [];
                items.find('[data-hubitat-bg-image]').each(function () {
                    var img = document.createElement('img');
                    img.src = $(this).attr('data-hubitat-bg-image');
                ...
                var checkImages = function () {
                    ...
                    if (result && result2) {
                        ...
                        if (container.hasClass('hubitat-masonry') || container.hasClass('masonry')) {
                        ...
                        handleHubitatBgImages();

                        if (currentPage >= parseInt(elem.attr('data-lazy-pages-count'))) {
                            elem.remove();
                            $('body').trigger('hubitat:lazy_load_complete');
                            $('body').trigger('hubitat:cursor_mouseleave');
                        } else {
    ...
    function handleStretchContent() {
        if (!$('.page-sidebar').length) {
            ...
            $('[data-vc-stretch-content="true"], [data-vc-full-width="true"], [data-hubitat-stretch-content="true"], .alignfull').each(function () {
                if (Clb.isRtl) {
                    $(this).css('right', '0');
    ...
    window.hubitatRowRefresh = handleStretchContent;

    function handleScrollEffects() {
        $('[data-hubitat-scroll-anim]').each(function () {
            var anim = $(this).attr('data-hubitat-scroll-anim');

            if ($(this).offset().top < ($(window).scrollTop() + $(window).height() - 50)) {
                $(this).removeClass(anim).removeAttr('data-hubitat-scroll-anim');
            }
        });
    }

    function handleHubitatHeight() {
        ...
        $('[data-hubitat-full-height]').each(function () {
    ...
    $('.woocommerce-product-search input[name=s]').on("keyup", function (event) {
        ...
        var data = {
            'action': 'hubitat_ajax_search',
            'search_query': $(this).val(),
            'search_term': $('select[name="search_term"]').val()
        };

        $.post(hubitatVariables.url, data, function (response) {
            $('.search_results').empty();
    ...
    function handleSingleProductGallery(scrollContainer, product) {
        ...
        if (productImages.length > 1 && !product.find('.product-image-dots').length) {
            ...
            if (!product.parents('.clb-popup-product').length) {
                var data = 'data-hubitat-content-scroll="#scroll-product"';
            }
    ...
    function SubscribeModal() {
        ...
        var data = {
            action: 'hubitat_subscribe_modal'
        };

        jQuery.post(hubitatVariables.url, data, function (data) {
    ...
    function handleSubscribeModal() {
        switch (hubitatVariables.subscribe_popup_type) {
            case 'time':
                setTimeout(function () {
                    SubscribeModal();
                }, +hubitatVariables.subscribe_popup_var * 1000);
                break;
            case 'scroll':
                var ckeck = true;
                $(window).on('scroll', function (e) {
                    ...
                    if (ckeck && scrollPercentRounded > hubitatVariables.subscribe_popup_var) {
                        SubscribeModal();
                        ckeck = false;
                    }
                });
    ...
    if (hubitatVariables.subscribe_popup_enable && !getCookie('subscribeCookie')) {
    ...
    $(window).on('hubitat:handle_portfolio_onepage_slider', handlePortfolioOnepageSlider);
    ...
    $(window).on('hubitat:handle_circle_progress_bar', handleCircleProgressBar);
    ...
    $(window).on('hubitat:handle_accordion_horizontal_box', handleHorizontalAccordion);
    ...
    function handleMasonry() {
        ...
        if ($('.hubitat-masonry').length) {
            setTimeout(function () {
                $('.hubitat-masonry').each(function () {
    ...
    $(window).on('hubitat:handle_masonry', handleMasonry);
    ...
    $(window).on('hubitat:handle_remove_slider_bullets', handleRemoveSliderBulletsClass);
    ...
    function handlePageColorSwitcher() {
        ...
        if (document.cookie == 'hubitat-swticher-state=dark') {
        ...
        switcher.on('click', function () {
            ...
            if (hubitatVariables && hubitatVariables.save_color_mode_state) {
                document.cookie = 'hubitat-swticher-state=' + (($(Clb.body).hasClass('dark-scheme')) ? 'dark' : 'light');
            }
    ...
    $(window).on('hubitat:handle_interactive_links_grid', handleInteractiveLinksGrid);
    ...
    $(window).on('hubitat:handle_portfolio_moving_details_grid', handlePortfolioMovingDetailsGrid);
    ...
    $(window).on('hubitat:handle_global_page_animation', handleGlobalPageAnimation);
    ...
    $(window).on('hubitat:portfolioGridType12', portfolioGridType12);
    ...
    function asymmetricParallaxGrid() {
        ...
        //     if (asymmetricGrid.hasClass('hubitat-masonry') || asymmetricGrid.data('isotope-grid') ) {
        //         setTimeout(function(){
    ...
    function handleHubitatBgImages() {
        $('[data-hubitat-bg-image]').each(function () {
            $(this).css('background-image', 'url(' + $(this).attr('data-hubitat-bg-image') + ')');
        });
    }
    $(window).on('hubitat:handle_bg_images', handleHubitatBgImages);

    function handleHubitatDynamicTextSc() {
    ...
    setTimeout(function () {
        $(window).on('hubitat:handle_hubitat_dynamic_text', handleHubitatDynamicTextSc);
    }, 1000)
    ...
    window.hubitatRefreshFrontEnd = function () {
        handleAccordionBox();
        handleBannerBox();
        handleBannerBoxSize();
        handleCounterBox();
    ...
    $(window).on('load', function () {
        ...
        $('body').on('click', '.notification-bar .clb-close', function (e) {
            ...
            setCookie('notification', 'enabled', +hubitatVariables.notification_expires);
    ...
    $(window).on('hubitat:handle_compare_shortcodes', handleCompareShortcodes);
    ...
    function handleCustomCursor() {
        ...
        if ($('body').hasClass('custom-cursor') && cursorInnerEl && cursorOuterEl) {
            ...
            $('body').on('mouseleave', '.cursor-magnet, .btn-round', hubitatRemoveMagneticFromCursor);

            function hubitatRemoveMagneticFromCursor() {
            ...
            $('body').on('hubitat:cursor_mouseleave', function () {
                hubitatRemoveMagneticFromCursor();
    ...
    if ($('body').hasClass('hubitat-anchor-onepage')) {
...
jQuery(window).on('elementor/frontend/init', function () {
    jQuery(window).trigger('hubitat:handle_global_page_animation');
});