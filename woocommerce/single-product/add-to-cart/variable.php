...
<form id="variation_form_anchor" class="woo_c-product-details-variations variations_form cart woo_c-cart-form" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo htmlspecialchars( json_encode( $available_variations ) ) ?>" data-please-select-message="<?php echo esc_attr__('Please select') ?>">
	...
	<?php if ( empty( $available_variations ) && false !== $available_variations ) : ?>
		<button type="submit" class="single_add_to_cart_button btn btn-small alt" disabled="true">
			<?php esc_html_e( 'This product is currently out of stock and unavailable.', 'hubitat' ); ?>
		</button>
	<?php else : ?>
		<div class="variations">
			<div class="variation">
				<div class="label">
					<label for="php"><?php esc_html_e( 'Quantity', 'hubitat' ); ?>:</label>
				</div>
			...
			<?php foreach ( $attributes as $attribute_name => $options ) : ?>
				...
				<?php
					echo end( $attribute_keys ) === $attribute_name ? apply_filters( 'woocommerce_reset_variations_link', '<div class="reset variation"><a class="btn btn-link reset_variations" href="#"><span>' . esc_html__( 'Reset', 'hubitat' ) . '</span><i class="ion ion-right ion-md-close-circle-outline"></i></a></div>' ) : '';
				?>
			<?php endforeach;?>
