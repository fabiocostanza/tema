...
<div class="vc_row" id="sticky-woo-sidebar">
						...
						if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
							...
							<div class="woocommerce-cart-form__cart-item woo-cart_item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
										...
										<?php
											echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
												...
												sprintf(
													...
													esc_html__( 'Remove this item', 'hubitat' ),
													esc_attr( $product_id ),
								...
								<div class="woo-cart_details">
									<div class="product-name woo-cart_item_name" data-title="<?php esc_attr_e( 'Product', 'hubitat' ); ?>">
										<div class="woo-c_product woo-c_product_name">
											...
											if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
												echo wp_kses_post( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'hubitat' ) . '</p>', $product_id ) );
											}
											?>	
										</div>
									...
									<div class="product-price woo-cart_item_price" data-title="<?php esc_attr_e( 'Price', 'hubitat' ); ?>">
										<?php
											echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key ); // PHPCS: XSS ok.
										?>
									</div>
									<div class="product-quantity woo-cart_item_quantity" data-title="<?php esc_attr_e( 'Quantity', 'hubitat' ); ?>">
										<?php
										...
										echo '<span class="quantity-label">' . esc_html_e( 'QTY:', 'hubitat' ) . '</span>';
										echo apply_filters( 'woocommerce_cart_item_quantity', $product_quantity, $cart_item_key, $cart_item ); // PHPCS: XSS ok.
										?>
									</div>
									<div class="product-subtotal woo-cart_item_subtotal" data-title="<?php esc_attr_e( 'Subtotal', 'hubitat' ); ?>">
										<?php
				...
				<div class="woo-c_actions actions">
					<?php if ( wc_coupons_enabled() ) { ?>
						<fieldset class="woo-c_actions_coupon coupon">
							<label for="coupon_code" class="field-label"><?php esc_html_e( 'Coupon code', 'hubitat' ); ?></label>
							<input type="text" name="coupon_code" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Paste coupon code here', 'hubitat' ); ?>" />
							<button type="submit" class="btn btn-flat" name="apply_coupon"><?php esc_attr_e( 'Apply Coupon', 'hubitat' ); ?></button>
							<?php do_action( 'woocommerce_cart_coupon' ); ?>
						</fieldset>
					<?php } ?>

					<button type="submit" class="button" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'hubitat' ); ?>"><?php esc_html_e( 'Update cart', 'hubitat' ); ?></button>
	...
	<div class="vc_col-lg-5 vc_col-md-4 vc_col-sm-12">
		<div data-hubitat-content-scroll="#sticky-woo-sidebar">
