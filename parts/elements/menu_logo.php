<?php
	$logo = HubitatSettings::get_logo();
	$logo_for_fixed = HubitatSettings::get_logo( false, true );
	$logo_as_image = is_array( $logo );
	$logo_for_fixed_as_image = is_array( $logo_for_fixed );
	$logo_for_onepage = HubitatSettings::get_logo_for_onepage();
	$header_style = HubitatOptions::get( 'page_header_menu_style', 'style1' );
	$logo_text = HubitatOptions::get( 'page_header_logo_text', get_bloginfo( 'name' ), false, true );
	$switcher = HubitatOptions::get( 'page_dark_mode_switcher', false );
	$dark_scheme = HubitatOptions::get( 'page_dark_mode', false );
	...
	if ($is_dark_scheme) {
		...
		if ($logo_type == "custom") {
			$_logo_light['default'] = HubitatOptions::get( 'page_header_custom_logo' );
		} else {
			if ( is_array( $_logo ) ) {
