...
<head>
	...
	<?php
		...
		// HubitatBuffer::start_content_bufferization();
...
<body <?php body_class(); ?>>
	...
	<div id="page" class="site">
		<a class="skip-link screen-reader-text" href="#main"><?php esc_html_e( 'Skip to content', 'hubitat' ); ?></a>
		<?php if ( HubitatOptions::get( 'page_use_boxed_wrapper', false ) ) : ?>
		<div class="boxed-container">
		<?php endif; ?>

		<?php if ( !HubitatHelper::is_optimized_flow( 'header' ) ): ?>

		<?php get_template_part( 'parts/headers/subheader' ); ?>

		<?php
			$header_menu_style = HubitatOptions::get( 'page_header_menu_style', 'style1' );
			$show_header = HubitatOptions::get( 'page_header_visibility', true ) && !HubitatSettings::page_is( 'for_builder' ) && !HubitatSettings::is_coming_soon_page();
			$append_header_cap = HubitatOptions::get( 'page_header_add_cap', true );
			$append_subheader = HubitatSettings::subheader_is_displayed();
			$show_search = HubitatOptions::get( 'page_header_search_visibility', true ) && !HubitatSettings::is_coming_soon_page();
			$mobile_menu = HubitatOptions::get_global( 'page_mobile_menu_initial_resolution' ); 
			$header_cap_class = '';
		...
		<div class="clb-popup clb-search-popup">
			...
			<div class="search-holder">
				<?php
				$search_type = HubitatSettings::get('page_header_search_type', 'global');
				if ($search_type == 'woo') {
