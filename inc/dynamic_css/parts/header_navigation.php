...
HubitatOptions::get( 'page_header_menu_style_settings' ); // trigger selection chain
$style_settings_type = HubitatOptions::get_last_select_type();
$style_select_type = HubitatOptions::get_last_select_type();

# 2. Background color and image
$background_type = HubitatOptions::get_by_type( 'page_header_menu_background_type', $style_settings_type );
if ( !$background_type ) $background_type = 'color';

$background_color = HubitatOptions::get_by_type( 'page_header_menu_background_color', $style_settings_type );

if ( $background_color ) {
	$background_color_css = 'background-color:' . $background_color . ';';
}
if ( $background_type == 'image' ) {
	$background_image_css = HubitatHelper::get_background_image_css_by_type( 'page_header_menu', $style_settings_type );
}

# --- Fixed header
$fixed_background_type = HubitatOptions::get_by_type( 'page_header_fixed_background_type', 'global' );
if ( !$fixed_background_type ) $fixed_background_type = 'color';

$fixed_background_color = HubitatOptions::get_by_type( 'page_header_fixed_background_color', 'global' );
...
if ( $fixed_background_type == 'image' ) {
	$fixed_background_image_css = HubitatHelper::get_background_image_css_by_type( 'page_header_fixed', 'global' );
}

# --- Mobile header
$mobile_background_type = HubitatOptions::get_by_type( 'page_mobile_header_menu_background_type', 'global' );
if ( !$mobile_background_type ) $mobile_background_type = 'color';

$mobile_background_color = HubitatOptions::get_by_type( 'page_mobile_header_menu_background_color', 'global' );
...
if ( $mobile_background_type == 'image' ) {
	$mobile_background_image_css = HubitatHelper::get_background_image_css_by_type( 'page_mobile_header_menu', 'global' );
}

$mobile_header_menu_color = HubitatOptions::get_global( 'page_mobile_header_menu_color', 'global' );
if ( $mobile_header_menu_color && $mobile_header_menu_color != 'global' ) {
	$mobile_header_menu_color_css .= 'color:' . $mobile_header_menu_color . ';';
}

$fixed_search_color = HubitatOptions::get( 'page_header_search_color', null, null, true );
if ( $fixed_search_color ) {
	$fixed_search_color_css .= 'color:' . $fixed_search_color . ';';
}

# --- Mobile fixed header
$mobile_background_color = HubitatOptions::get_global( 'page_mobile_header_menu_background' );
if ( $mobile_background_color ) {
	$mobile_background_color_css = 'background-color:' . $mobile_background_color . ';';
}
...
$header_typo = HubitatOptions::get_by_type( 'page_header_menu_text_typo', $style_settings_type );
$header_typo_css = HubitatHelper::parse_acf_typo_to_css( $header_typo );

$fixed_typo = HubitatOptions::get_global( 'page_header_fixed_text_typo' );
$fixed_typo_css = HubitatHelper::parse_acf_typo_to_css( $fixed_typo );

$mobile_typo = HubitatOptions::get_global( 'mobile_header_menu_typo' );
$mobile_typo_css = HubitatHelper::parse_acf_typo_to_css( $mobile_typo );
...
$border_visibility = HubitatOptions::get_by_type( 'page_header_menu_border_visibility', $style_settings_type, true );

$fixed_border_visibility = HubitatOptions::get_global( 'page_fixed_header_menu_border_visibility', false );
...
if ( $border_visibility ) {
	$border_type = HubitatOptions::get_by_type( 'page_header_menu_border_type', $style_settings_type, 'solid' );
...
if ( $fixed_border_visibility ) {
	$fixed_border_type = HubitatOptions::get_global( 'page_fixed_header_menu_border_type', 'solid' );
...
if ( $border_visibility ) {
	$border_color = HubitatOptions::get_by_type( 'page_header_menu_border_color', $style_settings_type );
...
if ( $fixed_border_visibility ) {
	$fixed_border_color = HubitatOptions::get_global( 'page_fixed_header_menu_border_color' );
...
$header_height = HubitatOptions::get_by_type( 'page_header_menu_height', $style_settings_type );

if ( $header_height ) {
	$header_height_css .= 'height:${height}px;';
	$header_height_css = HubitatHelper::parse_responsive_height_to_css( $header_height, $header_height_css );
}

$fixed_height = HubitatOptions::get_global( 'page_header_fixed_height' );

if ( $fixed_height ) {
	$fixed_height_css .= 'height:${height}px;';
	$fixed_height_css = HubitatHelper::parse_responsive_height_to_css( $fixed_height, $fixed_height_css );
}

# 5. Logo height

$logo_height = HubitatOptions::get_global( 'page_header_logo_height', $style_settings_type );

if ( $logo_height ) {
	$logo_height_css .= 'max-height:${height}px;';
	$logo_height_css = HubitatHelper::parse_responsive_height_to_css( $logo_height, $logo_height_css );
}

# 6. Site name typography

$logo_style = HubitatOptions::get( 'page_header_logo_style' );
$logo_style_select_type = HubitatOptions::get_last_select_type();

if ( $logo_style == 'sitename' ) {
	$sitename_typo = HubitatOptions::get_by_type( 'page_header_sitename_typo', $logo_style_select_type );
	$sitename_typo_css = HubitatHelper::parse_acf_typo_to_css( $sitename_typo );
}

# 7. Overlay background color and image

$overlay_background_type = HubitatOptions::get_global( 'page_header_overlay_menu_background_type' );
if ( !$overlay_background_type ) $overlay_background_type = 'color';

$overlay_background_color = HubitatOptions::get_global( 'page_header_overlay_menu_background_color' );
...
if ( $overlay_background_type == 'image' ) {
	$overlay_background_image_css = HubitatHelper::get_background_image_css_by_type( 'page_header_overlay_menu', 'global' );
}
...
if ( $header_typo_css ) {
	...
	HubitatBuffer::pack_dynamic_css_to_buffer( $_selector, $header_typo_css );
}

$custom_button_background = HubitatOptions::get_global('custom_button_for_header_background');
...
$custom_button_color = HubitatOptions::get_global('custom_button_for_header_color');
...
$subheader = HubitatOptions::get_global( 'page_subheader_visibility', true );
...
if ( $previous_btn && $header_height) {
	...
	if ($subheader) {
		$subheader_height = HubitatOptions::get_by_type( 'page_subheader_height', $style_select_type );
...
if ( $fixed_typo_css ) {
	...
	HubitatBuffer::pack_dynamic_css_to_buffer( $_selector, $fixed_typo_css );
}

if ( $mobile_color_css ) {
	$_selector = 'header#masthead';
	HubitatBuffer::pack_dynamic_css_to_buffer( $_selector, $mobile_color_css, 'mobile' );
}
...
if ( $mobile_header_menu_color_css ) {
	...
	HubitatBuffer::pack_dynamic_css_to_buffer( $_selector, $mobile_header_menu_color_css, 'mobile' );
}

if ( $fixed_search_color_css ) {
	$_selector = '.search-global.fixed .ion';
	HubitatBuffer::pack_dynamic_css_to_buffer( $_selector, $fixed_search_color_css );
}
...
if ( $mobile_typo_css ) {
	$_selector = [
		...
		'.header.-mobile .nav .menu .nav-item'
	];
	HubitatBuffer::pack_dynamic_css_to_buffer( $_selector, $mobile_typo_css );
}
...
if ( $border_css ) {
	...
	HubitatBuffer::pack_dynamic_css_to_buffer( $_selector, $border_css );
}

if ( $fixed_border_css ) {
	$_selector = '.header.-sticky';
	HubitatBuffer::pack_dynamic_css_to_buffer( $_selector, $fixed_border_css );
}
...
if ( $header_height_css ) {
	...
	if ( $header_height_css['desktop'] ) {
		...
		HubitatBuffer::append_to_dynamic_css_buffer( $_style_block, 'desktop' );
	}
	if ( $header_height_css['tablet'] ) {
		$_style_block = $header_height_classes . '{' . $header_height_css['tablet'] . '}';
		HubitatBuffer::append_to_dynamic_css_buffer( $_style_block, 'tablet' );
	}
	if ( $header_height_css['mobile'] ) {
		$_style_block = $header_height_classes . '{' . $header_height_css['mobile'] . '}';
		HubitatBuffer::append_to_dynamic_css_buffer( $_style_block, 'mobile' );
	}
}
...
if ( $back_link ) {
	...
	HubitatBuffer::append_to_dynamic_css_buffer( $_style_block);
...
if ( $sitename_typo_css ) {
	...
	HubitatBuffer::pack_dynamic_css_to_buffer( $_selector, $sitename_typo_css );
}
...
if ( $custom_button_color_css ) {
	...
	HubitatBuffer::pack_dynamic_css_to_buffer( $_selector, $custom_button_color_css );
}