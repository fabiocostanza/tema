...
class HubitatObjectParser {
    ...
    */
    static function parse_to_post_object( $_post, $_context = 'index', $_index = false ) {
        ...
        $post_object['overlay'] = HubitatOptions::get( 'post_overlay_color', null, $_post->ID );

        // formats
        $format = get_post_format( $_post->ID );

        $size = HubitatOptions::get_global( 'blog_images_size', 'medium_large' );
        ...
        */
        ...
        if ( $format == 'gallery' ) {
            ...
            if ( is_array( $galleries_matches ) && $galleries_matches ) {
                if(isset($galleries_matches[1])) {
                    ...
                    $post_object['media']['gallery'] = '<div data-cursor-class="cursor-link" class="slider blog-slider" data-hubitat-slider-simple="true">';
                    if ( $_post->metro ) {
                        foreach ( $galleries_matches as $attachment_ID ) {
                            $_img = wp_get_attachment_image_url( $attachment_ID, $size );
                            if ( $_img ) {
                                $post_object['media']['gallery'] .= '<div data-cursor-class="cursor-link" class="slider clb-slider-item blog-metro-image" data-hubitat-bg-image="' . esc_url( $_img ) . '">' . '</div>';
                            }
                        }
                    } else {
                        foreach ($galleries_matches as $attachment_ID) {
        ...
        // $layout = HubitatOptions::get( 'blog_item_layout_type', 'blog_grid_1', $_post->ID );

        // if ( $layout == 'blog_grid_1' || $layout == 'blog_grid_2') {
        // 	$boxed = HubitatOptions::get( 'items_boxed_style', null, $_post->ID );
        // 	$post_object['boxed'] = ( $boxed == 'default' );
        // }

        $post_object['grid_style'] = HubitatOptions::get( 'page_post_style_in_grid', 'default', $_post->ID );
        ...
        switch ( $_context ) {
            case 'index':
            default:
                $post_object['animation_type'] = HubitatOptions::get( 'page_animation_type' );
                if ( in_array( $post_object['animation_type'], array( 'sync', 'async' ) ) ) {
                    $post_object['animation_effect'] = HubitatOptions::get( 'page_animation_effect', 'fade-up' );
                }
                break;
        }

        // reading estimate
        $post_object['reading_estimate'] = HubitatHelper::get_reading_estimate( $_raw_post->post_content ?? '' );

        $post_object['metro_style'] = HubitatOptions::get( 'blog_metro_style', false );
    ...
    */
    static function parse_to_project_object( $_post ) {
        ...
        $show_description = HubitatOptions::get( 'project_show_description', true );
        // description
        if ( $show_description ) {
            $project_object['description'] = trim( HubitatOptions::get_local( 'project_description', $_post->ID ) );
            $project_object['short_description'] = '';
            if ( ! HubitatOptions::get_local( 'project_description', $_post->ID ) ) {
                $project_object['short_description'] = '';
            } else  {
                $_desc_first_space_arr = explode(' ', strip_tags( trim( HubitatOptions::get_local( 'project_description', $_post->ID ) ) ));
                $length = 24;
                if (HubitatSettings::get('portfolio_descr_length', 'global')) {
                    $length = HubitatSettings::get('portfolio_descr_length', 'global');
                }
        ...
        $show_lightbox_description = HubitatOptions::get( 'portfolio_gallery_description', true );
        ...
        if ( $show_lightbox_description ) {
            ...
            $_desc_first_space_arr = explode(' ', strip_tags( trim( HubitatOptions::get_local( 'project_description', $_post->ID ) ) ));
            $length = 24;
            if (HubitatSettings::get('portfolio_gallery_descr_length', 'global')) {
                $length = HubitatSettings::get('portfolio_gallery_descr_length', 'global');
            }
        ...
        // Visible
        $project_object['category_visible'] = HubitatOptions::get( 'portfolio_page_category_visibility', true );
        $project_object['more_visible'] = HubitatOptions::get( 'portfolio_page_more_visibility', true );

        // Images
        $gallery = HubitatOptions::get_local( 'project_content', $_post->ID );
        ...
        if ( is_array( $gallery ) && count( $gallery ) > 0 ) {
            foreach ( $gallery as $key => $value ) {
                ...
                if ( $value && (is_string( $value ) || is_numeric( $value ) ) ) {
                    ...
                    if ( isset( $value['sizes']['hubitat_full'] ) ) {
                        $_prepared_full_image['url'] = $value['sizes']['hubitat_full'];
                    } else {
        ...
        // Featured image
        $thumbnailSize = ( $_post->image_size ) ? $_post->image_size : HubitatOptions::get_global( 'portfolio_images_size', 'medium_large' );
        ...
        // Get hubitat_full size for full image
        $project_object['featured_image_full'] = get_the_post_thumbnail_url( $_post->ID, 'hubitat_full' );
        ...
        $video_url = HubitatOptions::get_local( 'project_video', $_post->ID );
        ...
        $project_object['featured_as_gallery_item'] = HubitatOptions::get( 'project_add_featured_to_gallery', true );

        $show_details = HubitatOptions::get( 'project_show_details', true );
        $show_link = HubitatOptions::get( 'project_show_link', true );
        $show_date = HubitatOptions::get( 'project_show_date', true );
		$show_task = HubitatOptions::get( 'project_show_task', true );
		$project_object['task'] = '';

        // info
        if ( $show_date ) $project_object['date'] = HubitatOptions::get_local( 'project_date', $_post->ID );
		if ( $show_task ) $project_object['task'] = HubitatOptions::get_local( 'project_task', $_post->ID );
        if ( $show_details ) {
            $project_object['strategy'] = HubitatOptions::get_local( 'project_strategy', $_post->ID );
            $project_object['design'] = HubitatOptions::get_local( 'project_design', $_post->ID );
            $project_object['client'] = HubitatOptions::get_local( 'project_client', $_post->ID );
        }
        if ( $show_link ) $project_object['link'] = HubitatOptions::get_local( 'project_link', $_post->ID );

        $tags = wp_get_post_terms($_post->ID, 'hubitat_portfolio_tags', array('fields' => 'names'));
        $project_object['tags'] = '';
        ...
        $project_object['raw_tags'] = wp_get_post_terms( $_post->ID, 'hubitat_portfolio_tags' );

        // custom info
        if ( $show_details ) {
            $project_object['custom_fields'] = array();
            $_custom_fields = HubitatOptions::get_local( 'project_custom_fields', $_post->ID );
        ...
        // show navigation
        $nav = HubitatOptions::get( 'project_show_navigation', true, $_post->ID );
        if ( !is_bool($nav) ) {
        ...
        $project_object['custom_content_position'] = HubitatOptions::get( 'project_custom_content_position', 'bottom', $_post->ID );

        // breadcrumbs
        $project_object['hide_breadcrumbs'] = HubitatOptions::get( 'page_show_breadcrumbs', true, $_post->ID );

        // sharing
        $project_object['show_sharing'] = HubitatOptions::get( 'project_sharing_buttons_visibility', true );
        $project_object['sharing_links'] = HubitatOptions::get_global( 'project_social_sharing_buttons' );
            ...
            foreach( $project_object['sharing_links'] as $share_link ) {
                ...
                if ( $share_link == 'twitter' ) : ?>
                    <a href="https://twitter.com/intent/tweet?text=<?php echo urlencode( $project_object['title'] ); ?>,+<?php echo rawurlencode( get_permalink() ); ?>" class="twitter">
                        ...
                        <span class="social-text"><?php esc_html_e( 'Twitter', 'hubitat' ); ?></span>
                    </a>
                <?php endif;
                ...
                if ( $share_link == 'facebook' ) : ?>
                    <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo rawurlencode( get_permalink() ); ?>" class="facebook">
                        ...
                        <span class="social-text"><?php esc_html_e( 'Facebook', 'hubitat' ); ?></span>
                    </a>
                <?php endif;
                ...
                if ( $share_link == 'linkedin' ) : ?>
                    <a href="https://www.linkedin.com/shareArticle?mini=true&url=<?php echo rawurlencode( get_permalink() ); ?>&title=<?php echo urlencode( $project_object['title'] ); ?>&source=<?php echo urlencode( get_bloginfo( 'name' ) ); ?>" class="linkedin">
                        ...
                        <span class="social-text"><?php esc_html_e( 'LinkedIn', 'hubitat' ); ?></span>
                    </a>
                <?php endif;
                ...
                if ( $share_link == 'pinterest' ) : ?>
                    <a href="http://pinterest.com/pin/create/button/?url=<?php echo rawurlencode( get_permalink() ); ?>&description=<?php echo urlencode( $project_object['title'] ); ?>" class="pinterest">
                        ...
                        <span class="social-text"><?php esc_html_e( 'Pinterest', 'hubitat' ); ?></span>
                    </a>
                <?php endif;
                ...
                if ( $share_link == 'vk' ) : ?>
                    <a href="http://vk.com/share.php?url=<?php echo rawurlencode( get_permalink() ); ?>" class="vk">
                        <i class="fab fa-vk"></i>
                        <span class="social-text"><?php esc_html_e( 'Vkontakte', 'hubitat' ); ?></span>
                    </a>
                <?php endif;
        ...
        // portfolio link
        $project_object['link_to_all'] = HubitatOptions::get_global( 'portfolio_page', home_url( '/' ) );
        ...
        // categories
        $show_categories = HubitatOptions::get( 'project_show_categories', true );
        if ($show_categories) {
            $_categories = wp_get_post_terms( $_post->ID, 'hubitat_portfolio_category' );
            ...
            if ( $project_object['categories_group'] && is_array( $project_object['categories_group'] ) && count( $project_object['categories_group'] ) > 0 ) {
                ...
                foreach ($project_object['categories_group'] as $category) {
                    $_project_categories[] = 'hubitat-filter-project-' . hash( 'md4', $category->slug, false );
                }
        ...
        // next n prev
        $project_object['next'] = HubitatHelper::projects_get_adjacent_post( $_post, 'next' );
        if ( !isset( $project_object['next']->ID ) ) {
            $project_object['next'] = HubitatHelper::projects_get_bounding_post( 'first' );
        }
        ...
        $project_object['prev'] = HubitatHelper::projects_get_adjacent_post( $_post, 'prev' );
        if ( !isset( $project_object['prev']->ID ) ) {
            $project_object['prev'] = HubitatHelper::projects_get_bounding_post( 'last' );
        }
        ...
        // overlay color
        $project_object['overlay'] = HubitatOptions::get_local( 'project_overlay_color', $_post->ID );
        if ( ! $project_object['overlay'] ) {
            $project_object['overlay'] = false;
        }

        // grid
        $project_object['grid_style'] = HubitatOptions::get_local( 'project_style_in_grid', $_post->ID );

        // animation
        $project['with_animation'] = HubitatOptions::get_local( 'global_portfolio_with_animation', 'option' );
        ...
        $project_object['popup_id'] = uniqid( 'hubitat-lightbox-' );
        switch ( HubitatOptions::get_local( 'project_open_type', $_post->ID ) ) {
    ...
    protected static function get_project_thumbnail_advanced($post, $thumbnail_size)
    {
        ...
        if (empty($image)) {
            $images = HubitatOptions::get_local( 'project_content', $post->ID );
