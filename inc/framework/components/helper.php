...
class HubitatHelper {
	...
	static function parse_acf_typo_to_css( $typo_object, $cond = false ) {
		...
		if ( $typo_object && is_object( $typo_object ) ) {
			...
			if ( isset( $typo_object->font_family ) && in_array( 'font-family', $result_fields ) ) {
				...
				$GLOBALS['hubitat_google_fonts'][] = $font_key_array;
			}
	...
	static function parse_google_fonts_to_query_string( $list ) {
		$fonts_type = HubitatOptions::get_global( 'page_font_type', 'google_fonts' );

		switch ($fonts_type) {
			case 'adobe_fonts':
				if ( $hubitat_adobekit_url = HubitatOptions::get_global( 'adobekit_url' ) ) {
					return '//use.typekit.net/' . $hubitat_adobekit_url . '.css';
				} else {
					return false;
				}
				break;
			...
			case 'manual_custom_fonts':
				return HubitatHelper::get_custom_fonts_url( $list );
				break;
	...
	*/
	static function parse_gallery_layout( $atts ) {
		$posts_layout_item = HubitatOptions::get( 'blog_item_layout_type', 'blog_grid_1' );
		$posts_layout_type = HubitatOptions::get( 'blog_metro_style' );
		...
		$layout = '<div class="slider blog-slider" data-hubitat-slider-simple="true">';
		...
		if ( $posts_layout_type ) {
			...
			foreach ($attach_ids as $attach_id) {
				$_url = wp_get_attachment_url( $attach_id );
				if ($_url) {
					$layout .= '<img class="full-width" src="' . esc_url( $_url ) . '" alt="' . esc_attr__( 'Gallery slide', 'hubitat' ) . '">';
				}
			}
		}
	...
	*/
	static function add_required_script( $key ) {
		$GLOBALS['hubitat_required_scripts'][] = $key;
	}
	...
	*/
	static function is_script_required( $key ) {
		if  ( is_array ($GLOBALS['hubitat_required_scripts'] ) ) {
			$list = array_unique( $GLOBALS['hubitat_required_scripts'] );
	...
	*/
	static function get_storage_item_data( ) {
		$temp_data = $GLOBALS['hubitat_storage_tmp_data'];
		$GLOBALS['hubitat_storage_tmp_data'] = false;
		return ( $temp_data ) ? $temp_data : false;
	}

	static function set_storage_item_data( $data ) {
		$GLOBALS['hubitat_storage_tmp_data'] = $data;
		return true;
	}
	...
	*/
	static function parse_iconset_to_url( $list ) {
		if ( is_array( $list ) && count( $list ) > 0 ) {
			...
			foreach ( $list as $font_class ) {
				...
				$icon_value = HubitatHelper::iconset_url_by_icon( $icon_key );
	...
	static public function get_background_image_css_by_type( $prefix, $select_type, $ignore_image = false, $image_size = false ) {
		...
		if ( !$ignore_image ) {
			$image = HubitatOptions::get_by_type( $prefix . '_background_image', $select_type );
			if ( $image ) {
				// ----------------------------------------------------------------
				if ( $image_size ) {
		...
		$image_size = HubitatOptions::get_by_type( $prefix . '_background_size', $select_type );
		switch ( $image_size ) {
			case 'auto':
		...
		if ( $image_size != '100per' || !$image_size ) {
			$image_position = HubitatOptions::get_by_type( $prefix . '_background_position', $select_type );
			...
			$image_repeat = HubitatOptions::get_by_type( $prefix . '_background_repeat', $select_type );
			switch ( $image_repeat ) {
				case 'repeat':
		...
		$image_attach = HubitatOptions::get_by_type( $prefix . '_background_attach', $select_type );
	...
	static public function get_custom_fonts_url( $list ) {
		$all_fonts = HubitatOptions::get_global('manual_custom_fonts');
		$enqueues = [];
		...
		// wp_add_inline_style( 'hubitat-style', implode( "\n", $faces ) );
		HubitatLayout::append_to_shortcodes_css_buffer( implode( "\n", $faces ) );
	...
	static public function projects_get_adjacent_post($_post, $direction = 'prev') {
		...
		$join  = apply_filters( "get_{$adjacent}_post_join", $join, $in_same_cat, $excluded_categories, 'hubitat_portfolio_category', $_post );
		$where = apply_filters( "get_{$adjacent}_post_where", $wpdb->prepare("WHERE p.post_date $op %s AND p.post_type = 'hubitat_portfolio' AND p.ID <> %d AND p.post_status = 'publish'", $current_post_date, $_post->ID), $in_same_cat, $excluded_categories, 'hubitat_portfolio_category', $_post );
	...
	static public function projects_get_bounding_post($type = 'first') {
		...
		$join  = apply_filters( "get_{$adjacent}_post_join", '', $in_same_cat, $excluded_categories, 'hubitat_portfolio_category', $post );
		$where = apply_filters( "get_{$adjacent}_post_where", "WHERE p.post_type = 'hubitat_portfolio' AND p.post_status = 'publish'", $in_same_cat, $excluded_categories, 'hubitat_portfolio_category', $post );
	...
	static public function calculate_custom_fonts_inline() {
		if ( isset($GLOBALS['hubitat_google_fonts']) ) {
			...
			if ( $fonts_type == 'manual_custom_fonts' ) {
				HubitatHelper::get_custom_fonts_url( $GLOBALS['hubitat_google_fonts'] );
			}
		}
	}
	...
	static public function is_optimized_flow($slug) {
		$slugs = isset( $_GET['hubitat_of'] ) ? explode(',', $_GET['hubitat_of']) : [];
