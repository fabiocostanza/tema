...
class HubitatSettings {
	...
	static function page_is( $key = false, $strictly = false ) {
		...
		if ( is_array( self::$page_is_keys ) ) {
			...
			if ( is_tax( 'hubitat_portfolio_category' ) ) {
				$cases[] = 'portfolio_category';
			}
			if ( is_tax( 'hubitat_portfolio_tags' ) ) {
				$cases[] = 'portfolio_tag';
			}
			if ( get_post_type() == 'hubitat_portfolio' ) {
				$cases[] = 'project';
			}
	...
	static function get_logo( $contrast = false, $fixed = false ) {
		...
		$logo_type = HubitatOptions::get( 'page_header_logo_style', 'sitename' );
		$logo_type_select_type = HubitatOptions::get_last_select_type();

		if ( $logo_type_select_type == 'global' && $logo_type == 'image' ) {
			$logo_type = HubitatOptions::get_global( 'page_header_logo_by_default', 'dark_variant' );
		}

		// Contrast logo
		if ( $contrast && $logo_type == 'dark_variant' ) {
			$_logo = HubitatOptions::get_global( 'page_header_logo_image' );
		...
		// Logo for fixed
		if ( $fixed ) {
			$fixed_type = HubitatOptions::get_global( 'page_header_logo_when_fixed', 'dark_variant' );
			switch ( $fixed_type ) {
				case 'dark_variant':
				...
				case 'custom':
					$_logo = HubitatOptions::get_global( 'page_header_logo_image_fixed_variant' );
					if ( is_array( $_logo ) ) {
		...
		switch ( $logo_type ) {
			case 'dark_variant':
				$_logo = HubitatOptions::get_global( 'page_header_logo_image_dark_variant' );
				if ( is_array( $_logo ) ) {
			...
			case 'light_variant':
				...
				break;
			case 'custom':
				$_logo = HubitatOptions::get( 'page_header_custom_logo' );
	...
	static function get_logo_for_onepage() {
		...
		$_custom_logo = HubitatOptions::get( 'page_header_custom_logo' );
	...
	static function footer_widget_logo() {
		...
		$logo_type = HubitatOptions::get( 'page_footer_logo_widget_type', 'light_variant' );
		$logo_type_select_type = HubitatOptions::get_last_select_type();
		...
		switch ( $logo_type ) {
			...
			case 'light_variant':
				...
				break;
			case 'custom':
				$_logo = HubitatOptions::get_by_type( 'page_footer_custom_logo', $logo_type_select_type );
				if ( $_logo ) {
	...
	static function header_subtitle_type() {
		$subtitle_type = HubitatOptions::get_local( 'page_header_title_subtitle_type' );
		if ( in_array( HubitatOptions::get( 'page_header_title_subtitle_type' ), ['inherit', NULL ] ) ) {
			$subtitle_type = HubitatOptions::get_global( 'post_subtitle_type', 'generated' );
		}
	...
	static function header_subtitle_custom_text() {
		...
		if ( HubitatOptions::page_is( 'single' ) ) {
			$subtitle_type = HubitatOptions::get( 'page_header_title_subtitle_type' );
			if ( $subtitle_type == 'custom' ) {
				//$custom_text = self::get( 'header_subtitle' );
				$custom_text = HubitatOptions::get( 'page_header_title_custom_subtitle_text' );
			} elseif ( in_array( HubitatOptions::get( 'page_header_title_subtitle_type' ), [ 'inherit', NULL ] ) ) {
				$subtitle_type = HubitatOptions::get_global( 'post_subtitle_type' );
				if ( $subtitle_type == 'custom' ) {
					$custom_text = HubitatOptions::get( 'post_custom_subtitle', 'global' );
				}
			}
		} else {
			$custom_text = HubitatOptions::get( 'page_header_title_custom_subtitle_text', '', null, true );
		}
	...
	static function subheader_is_displayed() {
		$add_subheader = HubitatOptions::get( 'page_subheader_visibility', true );
	...
	static function posts_per_page() {
		$posts_count = HubitatOptions::get( 'blog_posts_per_page', get_option( 'posts_per_page' ) );
	...
	static public function is_coming_soon_page() {
		$option_switched = HubitatOptions::get_global( 'coming_soon_switch', false );
