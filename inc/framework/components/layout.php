...
class HubitatLayout {
	...
	static function show_shortcodes_inline_css() {
		...
		echo HubitatLayout::get_shortcodes_css_buffer(false);
		echo '</style>';
	}
	...
	static function the_paginator_layout( $current_page, $all_pages, $align = 'left' ) {
		...
		if ( $current_page > 1 ) {
			...
			$layout .= '<i class="ion-left ion ion-md-arrow-back"></i> ' . esc_html__( 'Prev', 'hubitat' );
			$layout .= '</a></li>';
		}
		// other button layout
		if ( isset( $ranges ) ) {
		...
		if ( $current_page < $all_pages ) {
			$layout .= '<li><a href="' . esc_url( get_pagenum_link( $current_page + 1 ) ) . '" class="btn btn-link">';
			$layout .= esc_html__( 'Next', 'hubitat' ) . ' <i class="ion-right ion ion-md-arrow-forward"></i>';
			$layout .= '</a></li>';
		}
