...
class HubitatOptions {
	...
	static public function get_global( $key, $default_value = null ) {
		...
		if ( self::$use_cache && HubitatOptionsCache::has( 'global_' . $key ) ) {
			$option_value = HubitatOptionsCache::get( 'global_' . $key );
		} else {
			...
			if ( self::$use_cache ) {
				HubitatOptionsCache::set( 'global_' . $key, $option_value );
			}
		}
	...
	static function page_is( $key = null, $strictly = false ) {
		...
		if ( is_array( self::$page_is_keys ) ) {
			...
			if ( get_post_type( $post_id ) == 'hubitat_portfolio' ) {
				$cases[] = 'project';
			}
