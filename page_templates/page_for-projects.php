...
<?php
	...
	$GLOBALS['hubitat_icon_fonts'][] = 'my-icon-arr-out';
	HubitatHelper::add_required_script( 'masonry' );
	HubitatHelper::add_required_script( 'isotope' );

	get_header();

	$pagination_page = HubitatHelper::get_current_pagenum();
	$projects_per_page = (int)HubitatOptions::get( 'portfolio_projects_per_page', 12, false, true );
	$projects_offset = ( $pagination_page - 1 ) * $projects_per_page;

	$args = [
		'posts_per_page' => $projects_per_page,
		'offset' => $projects_offset,
		'post_type' => 'hubitat_portfolio',
		'post_status' => 'publish',
		'tax_query' => []
	];

	$allowed_categories_ids = [];
	$allowed_categories = HubitatOptions::get( 'portfolio_categories', [] );
	...
	// Filter
	$show_filter = HubitatOptions::get( 'project_page_filter_visibility', true );
	$filter_align = HubitatOptions::get( 'project_filter_align', 'center' );
	...
	// Popup
	$open_in_popup = HubitatOptions::get_global( 'portfolio_projects_in_popup' );
	$show_view_link_in_popup = HubitatOptions::get( 'portfolio_lightbox_link', true );
	$metro_style = HubitatOptions::get( 'portfolio_metro_style', false );
	...
	$animation_type = HubitatOptions::get( 'page_animation_type' );
	$animation_effect = HubitatOptions::get( 'page_animation_effect' );
	$animation_once = HubitatOptions::get( 'page_animation_once' );
	...
	$hover_effect = HubitatOptions::get( 'portfolio_grid_hover' );

	$boxed_classes = '';
	$boxed = HubitatOptions::get( 'portfolio_items_boxed_style', true );
	if ( $boxed ) {
		$boxed_classes = 'boxed';
	}
	...
	// Columns classes
	$columns_num = HubitatOptions::get_local( 'portfolio_columns_in_row' );
	
	if ( $columns_num == 'i-i-i') {
		$columns_num = HubitatOptions::get_global( 'portfolio_columns_in_row' );
	}

	$columns_num_global = HubitatOptions::get_global( 'portfolio_columns_in_row' );
	if ( ! $columns_num_global ) {
		$columns_num_global = '2-2-1';
	}

	$columns_class = HubitatHelper::parse_columns_to_css( $columns_num, false, $columns_num_global );
	$columns_double_class = HubitatHelper::parse_columns_to_css( $columns_num, true, $columns_num_global );
	...
	$asymmetric_parallax = HubitatOptions::get( 'grid_asymmetric_parallax', false );
	
	$asymmetric_atts = "";

	if ($asymmetric_parallax) {
		$asymmetric_parallax_speed = (int) HubitatOptions::get( 'asymmetric_parallax_speed', 20, false, true );
	...
	$projects_layout_item = HubitatOptions::get( 'portfolio_item_layout_type', 'grid_1' );
	$content_location = HubitatOptions::get_global( 'portfolio_content_position', 'top' );
	...
	if ( $is_slider ) {
		$navBtn = HubitatOptions::get( 'portfolio_nav_visibility' );
		$loop = HubitatOptions::get( 'portfolio_loop_mode' );
		$autoplay = HubitatOptions::get( 'portfolio_autoplay_mode' );
		$autoplayTimeout = HubitatOptions::get( 'portfolio_autoplay_interval', '', NULL, true );
		$mousewheel = HubitatOptions::get( 'portfolio_mousewheel_mode' );
		$dragscroll = HubitatOptions::get( 'portfolio_dragscroll_mode', true );
		$pagination = HubitatOptions::get( 'portfolio_bullets_visibility', true );
		$tilt_effect = HubitatOptions::get( 'portfolio_tilt_effect', true );

		$slider_pagination_data = '';
		if ($pagination) {
			$slider_pagination_type = HubitatOptions::get( 'portfolio_bullets_type' );
		...
		if ( $projects_layout_item === 'grid_6' ) {
			$slider_columns = HubitatOptions::get( 'portfolio_columns_in_row' );
			$slider_columns_global = HubitatOptions::get_global( 'portfolio_columns_in_row' );
	...
	$video_button_style = HubitatOptions::get( 'portfolio_video_button_style', 'default' );
?>
		...
		<div class="page-content<?php echo esc_attr( $sidebar_page_class ); ?>">
			<main id="main" class="site-main">
				...
				<?php if ( ! $is_slider ) : ?>
					...
					<div class="<?php echo esc_attr($projects_layout_item); ?>" data-hubitat-portfolio-grid="true" <?php echo esc_attr($grid_attr) ?>>
							...
							<?php if ( $show_filter) : ?>
								...
								<?php if ( is_array( $cat_ids ) && $cat_ids ) : ?>
									<div class="portfolio-sorting<?php echo esc_attr( $filter_align_class ); ?>" 
										...
										<ul class="unstyled">
											<li><?php esc_html_e( 'Filter by', 'hubitat' ) ?></li>
											<li>
												...
												<a class="active" href="#all" data-isotope-filter="*" data-category-count="<?php echo str_pad( (string)$published_projects, 2, '0', STR_PAD_LEFT ); ?>">
													<span class="name"><?php esc_html_e( 'All', 'hubitat' ); ?></span>
													<span class="num"><?php echo str_pad( (string)$published_projects, 2, '0', STR_PAD_LEFT ); ?></span>
												</a>
											</li>
											<?php foreach ( $cat_ids as $cat_obj ) : ?>
												<li> /
													<a href="#<?php echo esc_attr( $cat_obj->slug ); ?>" data-isotope-filter=".hubitat-filter-project-<?php echo hash( 'md4', $cat_obj->slug, false ); ?>" data-category-count="<?php echo esc_attr($cat_obj->count);?>">
							...
							foreach ( $projects_array as $_project_index => $_project_object ) {
								...
								$hubitat_project = HubitatObjectParser::parse_to_project_object( $_project_object );
								$hubitat_project['in_popup'] = $open_in_popup;
								if( $projects_layout_item == 'grid_1' || $projects_layout_item == 'grid_2' || $projects_layout_item == 'grid_11' || $projects_layout_item == 'grid_12'){
									$hubitat_project['boxed'] = $boxed_classes;
									$hubitat_project['hover_effect'] = $hover_effect;
									$hubitat_project['metro_style'] = $metro_style;
									$hubitat_project['video_button_style'] = $video_button_style;
								}
								HubitatHelper::set_storage_item_data( $hubitat_project );
								...
								if ($projects_layout_item != 'grid_8' && $projects_layout_item != 'grid_12') {
									...
									if ( $hubitat_project['grid_style'] == '2col') {
								...
								$_anim_attrs = HubitatHelper::get_AOS_animation_attrs( $animation_type, $animation_effect, (int) substr( $columns_in_row, 0, 1), $_post_i );

								echo '<div class="portfolio-item-wrap masonry-block' . esc_attr( $col_class ) . ' hubitat-project-item ' . 'grid-item section ' . esc_attr( $hubitat_project['categories_group'] ) . '" data-lazy-item="" data-lazy-scope="projects" >';
								...
								if ( $open_in_popup ) {
									$hubitat_project['show_view_link_in_popup'] = $show_view_link_in_popup;
									
									ob_start();
									HubitatHelper::set_storage_item_data( $hubitat_project );
									get_template_part( 'parts/portfolio_grid/lightbox' );
									HubitatLayout::append_to_footer_buffer_content( ob_get_clean() );
								}
					...
					<div class="vc_row wpb_row vc_row-fluid vc_row-no-padding portfolio-onepage-slider clb-slider-scroll-bar <?php echo esc_attr($projects_layout_item); ?>" 
						...
						<?php 
							...
							foreach ( $projects_array as $_project_index => $_project_object ) {
								...
								$hubitat_project['video_button_style'] = $video_button_style;
								if ($_project_index == 0) {
									$_last_project = HubitatObjectParser::parse_to_project_object( $projects_array[count($projects_array) - 1] );
									$hubitat_project['prev_item_featured_image'] = $_last_project['featured_image'];
								} else {
									$hubitat_project['prev_item_featured_image'] = $_prev_item_featured_image;
								}
								$_prev_item_featured_image = $hubitat_project['featured_image'];
								HubitatHelper::set_storage_item_data( $hubitat_project );
