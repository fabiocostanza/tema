...
<?php
	...
	$alignment = HubitatOptions::get( 'posts_text_alignment' );
	$hover_effect = HubitatOptions::get( 'blog_item_hover_type', 'type1' );
	...
	$posts_grid = HubitatOptions::get( 'blog_page_layout', 'masonry' );
	if ( $posts_grid == 'masonry' ) {
		HubitatHelper::add_required_script( 'masonry' );
	}
	$grid_style_class = ( $posts_grid == 'masonry' ) ? 'blog-posts-masonry hubitat-masonry' : 'blog-posts-classic';
	...
	$columns_num = HubitatOptions::get_local( 'blog_columns_in_row' );
	...
	if ( $columns_num == "i-i-i" ) $columns_num = HubitatOptions::get_global( 'blog_columns_in_row', '2-2-1' );
	$columns_num_global = HubitatOptions::get_global( 'blog_columns_in_row', '2-2-1' );
	...
	$animation_type = HubitatOptions::get( 'page_animation_type', 'default' );
	$animation_effect = HubitatOptions::get( 'page_animation_effect', 'fade-up' );
	...
	$content_location = HubitatOptions::get_global( 'post_content_position', 'top' );
?>
		...
		<div class="page-content<?php echo esc_attr( $sidebar_page_class ); ?>">
			<main id="main" class="site-main">
				...
				foreach ( $posts_array as $post_index => $post ) {
					...
					$_parsed_post = HubitatObjectParser::parse_to_post_object( $post, NULL, $_post_i + 1 );
					...
					HubitatHelper::set_storage_item_data( $_parsed_post );
					...
					$_anim_attrs = HubitatHelper::get_AOS_animation_attrs( $animation_type, $animation_effect, (int) substr( $columns_num, 0, 1), $_post_i );

					echo '<div class="' . esc_attr( $col_class . $grid_class . ( ( $posts_grid == 'masonry' ) ? ' hubitat-card-wrapper blog-post-masonry masonry-block' : '' ) ) . '" ' . implode( ' ', $_anim_attrs ) . ' data-lazy-item="" data-lazy-scope="posts">';
				...
				if ( $paginator_all > 1 ) {
					...
					switch ( HubitatOptions::get( 'pagination_type', 'simple' ) ) {
						case 'simple':
