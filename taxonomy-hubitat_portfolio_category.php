<?php
    ...
    $grid_style_class = 'hubitat-masonry blog-posts-masonry';

    $projects_layout_item = HubitatOptions::get_global( 'portfolio_item_layout_type', 'grid_1' );

    // Columns
    $columns_num = HubitatOptions::get_global( 'portfolio_columns_in_row', '2-2-1' );
    ...
    $grid_offset_class = '';
    $posts_without_paddings = HubitatOptions::get_global( 'portfolio_grid_items_without_padding', false );
    ...
    $metro_style = HubitatOptions::get( 'portfolio_metro_style' );
    $boxed = HubitatOptions::get( 'portfolio_items_boxed_style', true );
    if ( !is_bool( $boxed ) ) {
        if ( $boxed == 'default' ) $boxed = false;
        else $boxed = true;
    }
    $show_view_link_in_popup = HubitatOptions::get_global( 'portfolio_lightbox_link', true );
    $open_in_popup = HubitatOptions::get_global( 'portfolio_projects_in_popup', true );
    $alignment = HubitatOptions::get_global( 'projects_text_alignment', 'left' );
    $hover_effect = HubitatOptions::get_global( 'portfolio_grid_hover', 'global' );
    $animation_type = HubitatOptions::get( 'page_animation_type', 'sync' );
                ...
                <main id="main" class="site-main">
                    <div class="vc_row portfolio-grid <?php echo esc_attr( $grid_style_class . $grid_offset_class ); ?>" data-lazy-container="posts">
                        ...
                        while ( have_posts() ) : the_post();
                            ...
                            $parsed_post = HubitatObjectParser::parse_to_project_object( $post, NULL, $_post_i + 1 );
                            ...
                            echo '<div class="portfolio-item-wrap masonry-block hubitat-project-item grid-item section' . esc_attr( $col_class . $grid_class . $grid_offset_class ) . '" ' . implode( ' ', $_anim_attrs ) . ' data-lazy-item="" data-lazy-scope="posts">';
